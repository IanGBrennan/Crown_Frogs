...
source("~/Documents/GitHub/Crown_Frogs/Condensing_Alignments.R")
source("~/Documents/GitHub/Crown_Frogs/combineOrthologues.R")

setwd("~/Documents/GitHub/Crown_Frogs/Anura_Outgroups/Alignments")
dropEmptyTaxa(file.extension = ".fasta")

#setwd("~/Desktop/Myobatrachidae/Alignments")
#setwd("~/Desktop/Microhylidae/Alignments")
#setwd("~/Desktop/Hylidae/Alignments")
setwd("~/Desktop/MM_Alignments/Combined")
#setwd("~/Desktop/Anura_Outgroups/Alignments")
#setwd("~/Desktop/HH_Alignments/Combined")
...
rbh <- blast_best_reciprocal_hit(
  query = "~/Documents/GitHub/Crown_Frogs/Anura_Outgroups/Microhylidae_All_Loci.fasta",
...
# Compare the Hime and Hylidae alignments
rbh <- blast_best_reciprocal_hit(
  query = "~/Documents/GitHub/Crown_Frogs/Anura_Outgroups/Anura_Outgroups_All_Loci.fasta",
  subject = "~/Documents/GitHub/Crown_Frogs/Hylidae/Hylidae_All_Loci.fasta",
  search = "nucleotide_to_nucleotide",
  task = "dc-megablast",
  output.path = tempdir(),
  db.import = F
)
write.csv(rbh, file="~/Documents/GitHub/Crown_Frogs/RBH_Hime_Hylidae.csv", row.names=F, quote=F)
rbh.fasta <- rbh; rbh.fasta$query_id <- paste0(rbh.fasta$query_id,".fasta"); rbh.fasta$subject_id <- paste0(rbh.fasta$subject_id,".fasta")

# Combine our Hime alignments with the Hylidae alignments
setwd("~/Desktop/HH_Alignments")
combineOrthologues(filetype=".fasta", blast.table=rbh, path="~/Desktop/HH_Alignments")

# Which Hime loci weren't represented in the Hylidae?
Hi.files <- dir("~/Desktop/HH_Alignments", pattern="Hime")
excluded.Hi <- setdiff(Hi.files, rbh.fasta$query_id)
# Now copy them into the alignments directory
for (k in 1:length(excluded.Hi)){
  move.call <- paste("cp", paste0(getwd(),"/",excluded.Hi[[k]]), paste0(getwd(),"/Combined"))
  system(move.call)
}

# Which Hylidae loci weren't represented in the Hime?
Hy.files <- dir("~/Desktop/HH_Alignments", pattern="Hylidae")
excluded.Hy <- setdiff(Hy.files, rbh.fasta$subject_id)
# Now copy them into the alignments directory
for (k in 1:length(excluded.Hy)){
  move.call <- paste("cp", paste0(getwd(),"/",excluded.Hy[[k]]), paste0(getwd(),"/Combined"))
  system(move.call)
}



# Compare the HH and MM alignments
rbh <- blast_best_reciprocal_hit(
  query = "~/Documents/GitHub/Crown_Frogs/MM_Alignments/MM_All_Loci.fasta",
  subject = "~/Documents/GitHub/Crown_Frogs/HH_Alignments/HH_All_Loci.fasta",
  search = "nucleotide_to_nucleotide",
  task = "dc-megablast",
  output.path = tempdir(),
  db.import = F
)
write.csv(rbh, file="~/Documents/GitHub/Crown_Frogs/RBH_MM_HH.csv", row.names=F, quote=F)
rbh.fasta <- rbh; rbh.fasta$query_id <- paste0(rbh.fasta$query_id,".fasta"); rbh.fasta$subject_id <- paste0(rbh.fasta$subject_id,".fasta")

# Combine our MM alignments with the HH alignments
setwd("~/Desktop/MMHH_Alignments")
combineOrthologues(filetype=".fasta", blast.table=rbh, path="~/Desktop/MMHH_Alignments")

# Which MM loci weren't represented in the HH?
MM1.files <- dir("~/Desktop/MMHH_Alignments", pattern="Microhylidae")
MM2.files <- dir("~/Desktop/MMHH_Alignments", pattern="Mi_")
MM3.files <- dir("~/Desktop/MMHH_Alignments", pattern="Myobatrachidae")
MM.files <- c(MM1.files, MM2.files, MM3.files)
excluded.MM <- setdiff(MM.files, rbh.fasta$query_id)
# Now copy them into the alignments directory
for (k in 1:length(excluded.MM)){
  move.call <- paste("cp", paste0(getwd(),"/",excluded.MM[[k]]), paste0(getwd(),"/Combined"))
  system(move.call)
}

# Which Hylidae loci weren't represented in the Hime?
HH1.files <- dir("~/Desktop/MMHH_Alignments", pattern="Hylidae")
HH2.files <- dir("~/Desktop/MMHH_Alignments", pattern="Hime")
HH3.files <- dir("~/Desktop/MMHH_Alignments", pattern="Hi_")
HH.files <- c(HH1.files, HH2.files, HH3.files)
excluded.HH <- setdiff(HH.files, rbh.fasta$subject_id)
# Now copy them into the alignments directory
for (k in 1:length(excluded.HH)){
  move.call <- paste("cp", paste0(getwd(),"/",excluded.HH[[k]]), paste0(getwd(),"/Combined"))
  system(move.call)
}

# Rename all the combined MMHH alignments
setwd("~/Desktop/MMHH_Alignments/Combined")
bads <- dir(getwd(), pattern=".fasta")
for (y in 1:length(bads)){
  new.name <- paste0("Crown_Oz_L",y,".fasta")
  name.call <- paste("mv", paste0(getwd(),"/",bads[[y]]), paste0(getwd(),"/",new.name))
  system(name.call)
}
